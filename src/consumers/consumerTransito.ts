...
async function startTaskTransitoConsumer(): Promise<void> {
  try {
    ...
    const queue: string = enumMessengerCoda.queueTransito;
    ...
    channel.consume(
      ...
      async (msg: Message | null) => {
        if (msg) {
          ...
          console.log(
            enumMessengerCoda.queueTransito + ': Ricevuto: ${content}',
          );

          const objTransito: eTransito = eTransito.fromJSON(content);
          if (objTransito) {
            // ...
...
export default startTaskTransitoConsumer;
